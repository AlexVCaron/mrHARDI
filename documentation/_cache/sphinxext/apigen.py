...
from autogen_config import reverse_aliases, class_config_rst_doc
...
class FuncClsScanner(ast.NodeTransformer):
    ...
    def visit_ClassDef(self, node):
        if not (node.name.startswith('_') or self.has_undoc_decorator(node)) \
            ...
            try:
                cls.import_string = node.parent.import_string
                mod = import_module(node.parent.import_string)
                bases = getattr(mod, cls.name).mro()
                cls.is_app = any(
                    b.__name__ == 'MagicMonkeyBaseApplication' for b in bases
                )
            except AttributeError:
                cls.import_string = None
            self.classes.append(cls)
            self.classes_seen.add(node.name)

    def _add_parent(self, node, parent):
        node.parent = parent
        return node
    ...
    def visit(self, node):
        ...
        if isinstance(visitor, ast.AST):
            visitor.parent = node
        elif isinstance(visitor, (list, tuple, Generator)):
            visitor = (self._add_parent(n, node) for n in visitor)
        return visitor
                ...
                for value in old_value:
                    if isinstance(value, ast.AST):
                        ...
                        if value is None:
                            ...
                            new_values.extend(
                                (self._add_parent(n, node) for n in value)
                            )
                            continue
                    new_values.append(self._add_parent(value, node))
                old_value[:] = new_values
            elif isinstance(old_value, ast.AST):
                old_value.parent = node
    ...
    def generate_api_doc(self, uri):
        ...
        for c in classes:
            ...
            if c.has_init:
                  ad += '\n  .. automethod:: __init__\n'
            if c.is_app:
                mod = import_module(c.import_string)
                app = getattr(mod, c.name).instance()
