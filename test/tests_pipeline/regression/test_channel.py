...
class RegressionTestChannel(TestCase):
    def setUp(self):
        ...
        self.channel = Channel(self.loop, ["data"])
    ...
    def _test_yield_then_shutdown(self, n=1, task_creator=None):
        ...
        self.loop.run_until_complete(self.channel.wait_for_completion())
        self.loop.run_until_complete(future)
        self._wait_for_output_shutdown()
        self.loop.stop()
    ...
    def _wait_for_output_shutdown(self):
        while self.sub_out.running():
            self.loop.run_until_complete(asyncio.sleep(1, loop=self.loop))
