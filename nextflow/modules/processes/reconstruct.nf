...
process scilpy_response {
    ...
    script:
        ...
        scil_compute_ssst_frf.py $dwi $bval $bvec ${sid}__response.txt --mask $mask --fa $params.frf_fa --min_fa $params.frf_min_fa --min_nvox $params.frf_min_nvox --roi_radii $params.frf_roi_radius
        """
}
...
process scilpy_csd {
    ...
    script:
        ...
        export OMP_NUM_THREADS=1
        export OPENBLAS_NUM_THREADS=1
        scil_compute_ssst_fodf.py $dwi $bval $bvec $response ${sid}__fodf.nii.gz --mask $mask --force_b0_threshold --processes $task.cpus
        """
}
